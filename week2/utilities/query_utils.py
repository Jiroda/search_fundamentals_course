...
def add_click_priors(query_obj, user_query, priors_gb):
    try:
        ...
        if prior_clicks_for_query is not None and len(prior_clicks_for_query) > 0:
            ...
            item_boost_dictionary = (prior_clicks_for_query.sku.value_counts() / len(prior_clicks_for_query)).to_dict()
            click_prior = " ".join([f'{sku}^{boost}' for sku, boost in item_boost_dictionary.items()])
            if click_prior != "":
